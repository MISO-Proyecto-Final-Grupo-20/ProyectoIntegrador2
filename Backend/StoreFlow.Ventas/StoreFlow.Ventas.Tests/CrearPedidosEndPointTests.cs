...
public class CrearPedidosEndPointTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _app = TestApplicationFactory.Create(_publishEndpointMock);
    ...
    public async Task CrearPedido_RetornaAccepted()
    {
        ...
        var response = await _client.PostAsJsonAsync("/pedido", new CrearPedidoRequest(1, 
        [
            new ProductoPedidoRequest("1", 2, 10),
            new ProductoPedidoRequest("2", 3, 20)
        ]));
        
        await _publishEndpointMock!.Received(1).Publish(Arg.Any<ProcesarPedido>());
